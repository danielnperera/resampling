...
def calculate_equation_weights(errorset, dweightset, calculate_variance):
    # inverts it too
    n = errorset.size
    weights = np.empty(n)
...
def get_fit_range_with_error(threshold, data, error, mask):
    n_sets, n = mask.shape
    sigma = np.empty(n_sets)
    counts = np.empty(n_sets, dtype=nb.int64)
    for i in range(n_sets):
...
def asymmetric_distance_weights(offsets, alpha):
...
def symmetric_distance_weights(distance, alpha):
    """w = 1 / exp(-r^2 / alpha^2)"""
    n = distance.size
    weights = np.empty(n)
    for i in range(n):
...
def line_max_order(order, center, coordinates, required):
    left = 0
    right = 0
    left_found = False
    right_found = False
...
def symmetric_counts_maxorder(counts_array, order, features, required):
...
def asymmetric_counts_maxorder(counts_array, order, features):
